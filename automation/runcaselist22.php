<?php
	...
	try { 
		...
		file_put_contents("log.txt", "\n table:".$json_array->table, FILE_APPEND);
		...
		unset($projectarry);
		file_put_contents("log.txt", "\n caselist:".json_encode($json_array->caselist), FILE_APPEND);
		$test_list="";
		# 去掉[]
		...
		$test_list.="0";
		file_put_contents("log.txt", "\n caselist:".$test_list, FILE_APPEND);
		...
		for($m=1;$m<= intval($json_array->looptimes);$m++) {
			try { 
				...
				file_put_contents("log.txt", "\n resultlist:".$resultlist, FILE_APPEND);
					...
					try { 
						...
						if($array[$i]) {
							...
							if ($loginflag) {
								...
								stream_set_blocking($err_stream, true);
								stream_set_blocking($dio_stream, true);
								file_put_contents("log.txt", "\n 66666666666666:", FILE_APPEND);
								$result_err = stream_get_contents($err_stream);
								file_put_contents("log.txt", "\n 999999999:", FILE_APPEND);
								file_put_contents("log.txt", "\n result_err:".$result_err, FILE_APPEND);
								$result_dio = stream_get_contents($dio_stream); #获取流的内容，即命令的返回内容
								file_put_contents("log.txt", "\n result_dio:".$result_dio, FILE_APPEND);
								
							}
							file_put_contents("log.txt", "\n 777777777777:", FILE_APPEND);
							$datetime=date('Y-m-d h:i:s', time()); 
							$execution_ts=$datetime;
							$end_ts=$datetime;
							file_put_contents("log.txt", "\n execution_ts:".$execution_ts, FILE_APPEND);
							#保存文件
