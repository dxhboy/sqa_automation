	...
	if(count($array)<1) {
		...
		for($i=0;$i<count($array);$i++)
		{
			...
			if($array[$i]){//如果数据存在输出数据
				...
				$automatic_sql= "SELECT t.execution_ts,t.end_ts,t.status,t.log,t.details,t.notes,t.id FROM ( SELECT * FROM executions where testplan_id=".$json_array->id." and build_id='".$build_id."' and tcversion_id in (SELECT id from nodes_hierarchy where node_type_id='4' and parent_id=".$arr[0]." ) and execution_type='2' order by id) t order by t.execution_ts DESC;";
				// file_put_contents("log.txt", "sql11:\n $automatic_sql \n", FILE_APPEND);
