				...
				for($i=0;$i<count($array);$i++) {
					...
					try { 
						...
						if($array[$i]) {
							...
							if (!$loginflag) {
								$kill_cmd = "kill -9 $(ps -ef | grep ".$json_array->environment."|grep -v grep|awk '{print $2}')";
								$txt =json_encode($kill_cmd,JSON_UNESCAPED_UNICODE);
								echo($txt);
								echo('      ');
								...
								$txt =json_encode($config,JSON_UNESCAPED_UNICODE);
								echo($txt);
								echo('      ');
								...
								$txt =json_encode($command,JSON_UNESCAPED_UNICODE);
								echo($txt);
								echo('      ');
								$stream = ssh2_exec($connection, $kill_cmd);
								$stream = ssh2_exec($connection, $config);  #执行结果以流的形式返回
...
############################################';
				...
				if($json_array->sentmail=="on") {
					...
					$subject.=' Version:'.$json_array->edition.' '.$tester_name[0][0].' '.$Verdict.' Executor:'.$tester_name[0][1].$tester_name[0][2].'Passed('.$Passed.')Failed('.$Failed.')Aborted('.$Aborted.')Canceled('.$Canceled.')';
