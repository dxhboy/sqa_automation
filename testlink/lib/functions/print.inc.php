...
function renderTestCaseForPrinting(&$db,&$node,&$options,$env,$context,$indentLevel) {
        ...
        if (!is_null($tcInfo[$key]) && $tcInfo[$key] != '') {
          ...
          for($ydx=0 ; $ydx < $loop2do; $ydx++)
          {
           ...
           // '<td>' . ($stepDesignType == 'none' ? nl2br($tcInfo[$key][$ydx]['actions']) : $tcInfo[$key][$ydx]['actions'] ) . '</td>' .
			...
			// $expected_results=($stepDesignType == 'none' ? nl2br($tcInfo[$key][$ydx]['expected_results']) : $tcInfo[$key][$ydx]['expected_results'] );
			// $expected_results = str_replace(array("\r\n", "\r", "\n"), "</br>", $expected_results);
           ...
           '<td >' . ($stepDesignType == 'none' ? nl2br($tcInfo[$key][$ydx]['actions']) : $tcInfo[$key][$ydx]['actions'] ) . '</td>' .
            ...
            {   
				if( $nike )
				{
					...
					{
						$stepstatus = $statusL10N[$sk['status']];
						
					}  
				}  
				if( $stepstatus=="通过") {
					$code .= '<td> <font color="green">'.$stepstatus.'</td>';
					
				} else {
					$code .= '<td> <font color="red">'.$stepstatus.'</td>';
				}
                // $code .= '<td> <font color="red">';
				// if( $nike )
				// {
					// $sk = $sxni[$tcInfo[$key][$ydx]['id']];
					// if(isset($statusL10N[$sk['status']]))
					// {	
						...
						// file_put_contents("log.txt", $statusL10N[$sk['status']], FILE_APPEND);
  ...
  $tex = "id" .  $tprojectID . "tcversion_id" .   $platform_id . "level" .$build_id . "$designCfg";
...
/*
 * add Remarks function for testlinkInitPage()
 */
function addRemarks(&$dbHandler,$tcversion_id,$tplan_id)
{
  if(is_null($context))
  {
