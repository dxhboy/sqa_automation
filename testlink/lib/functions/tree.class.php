...
class tree extends tlObject
{
  ...
  */
  function new_node($parent_id,$node_type_id,$name='',$node_order=0,$node_id=0) 
  {
	...
	{
	  $sql .= ") {$values} )";
	  $sql1="update  nodes_hierarchy SET id=key_id where node_type_id ='1';";
	}
	else
	{
	  $sql .= ",parent_id) {$values},{$parent_id})";
	  $sql1="update  nodes_hierarchy SET id=key_id where parent_id =$parent_id;";
	}
	...
	$key1_id=$this->db->insert_id($this->object_table);
	
	
	
	$this->db->exec_query($sql1);
	session_start();
	$_SESSION['key1_id'] = $key1_id;
	// return ($this->db->insert_id($this->object_table));
	return $key1_id;
   }
  ...
  */
  function get_node_hierarchy_info($node_id,$parent_id = null,$options=null)
  {
	...
	if(strlen($result['node_type_id'])!=1) {
		if(strlen($result['parent_id'])<1) {
			$sql="SELECT t.id,t.name,t.parent_id,t.node_type_id,t.node_order FROM nodes_hierarchy t JOIN nodes_steps n on n.parent_id=t.id JOIN tcsteps tc on tc.parent_id=n.id WHERE tc.id=".$node_id.";";
			$rs = $this->db->get_recordset($sql);
			$result = !is_null($rs) ? $rs[0] : null;
		}
	}
  ...
  */
  function get_children($id,$exclude_node_types=null,$opt=null) {
    ...
    // file_put_contents("log.txt", "get_children:".$sql."\n", FILE_APPEND);  
    $node_list=array();  
    $result = $this->db->exec_query($sql);
  ...
  */
  ...
  function _get_subtree($node_id,&$node_list,$filters = null, $options = null)
  {
    ...
    switch($my['options']['order_cfg']['type'])
    {
      ...
      case 'exec_order':
		...
		// file_put_contents("log.txt", "_get_subtree:".$sql."\n", FILE_APPEND); 
      $result = $this->db->exec_query($sql);
  ...
  function _get_subtree_rec($node_id,&$pnode,$filters = null, $options = null)
  {
    ...
    // file_put_contents("log.txt", "_get_subtree_rec:".$sql."\n", FILE_APPEND); 
      $result = $this->db->exec_query($sql);
      $qnum++;
      while($row = $this->db->fetch_array($result))
      {
  ...
  function get_full_path_verbose(&$items,$options=null) {
      ...
      { 
		...
		// file_put_contents("log.txt", "get_full_path_verbose:".$sql."\n", FILE_APPEND); 
  ...
  */
  ...
  function getNodeByAttributes($attr)
  {
    ...
    $sql .= $addJoin . $where;
	// file_put_contents("log.txt", "getAllItemsID:".$sql."\n", FILE_APPEND);
