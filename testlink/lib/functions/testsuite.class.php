...
class testsuite extends tlObjectWithAttachments
{
  ...
  */
  ...
  /*
    function: exportTestSuiteDataToWord
    ...
    @internal revisions
  */
  function exportTestSuiteDataToWord($container_id,$tproject_id,$optExport = array())
  {
    static $keywordMgr;
    static $getLastVersionOpt = array('output' => 'minimun');
    static $tcase_mgr;
    ...
    if($doRecursion)
    {
	  ...
	  if (isset($optExport['ATTACHMENTS']) && $optExport['ATTACHMENTS'])
      {
		...
		// $xmlTC = '<testsuite id="' . $tsuiteData['id'] . '" ' .
               ...
               // "\n<node_order><![CDATA[{$tsuiteData['node_order']}]]></node_order>\n" .
               // "<details><![CDATA[{$tsuiteData['details']}]]></details> \n{$kwXML}{$cfXML}{$attachmentsXML}";
		$xmlTC = "\n" .'<h1 align="center"><font size="4" face="Cambria" color="black">' . htmlspecialchars($tsuiteData['name']). '</h1>'. "\n" ;
		// file_put_contents("log.txt", "\n details:\n".json_encode($tsuiteData["details"]), FILE_APPEND);
		// if(json_encode($tsuiteData["details"])!="null") {
		
			// $xmlTC .='<h3 align="left"><font size="4" face="Cambria" color="black">details</h3>'."\n".
						// "\t".'<p align="left"><font size="2" face="Calibri" color="black">&emsp;'. htmlspecialchars($tsuiteData['details']).'</p>'."\n";
		// }
    ...
    if( !is_null($childNodes) )
    {
      ...
      for($idx = 0;$idx < $loop_qty;$idx++)
      {
        ...
        if ($doRecursion && $nTable == 'testsuites')
        {
          $xmlTC .= $this->exportTestSuiteDataToWORD($cNode['id'],$tproject_id,$optExport);
        }
        else if ($nTable == 'testcases')
        {
          if( is_null($tcase_mgr) )
          ...
          $xmlTC .= $tcase_mgr->exportTestCaseDataToWORD($cNode['id'],testcase::LATEST_VERSION,
                                                        $tproject_id,true,$optExport);
