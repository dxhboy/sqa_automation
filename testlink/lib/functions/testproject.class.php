...
function create($item,$opt=null) {
  ...
  if($this->object_table=='testprojects') {
	   
		session_start();
		$key1_id = $_SESSION['key1_id'];
		if($id!=$key1_id) {
		 
			$id = $key1_id;
		}
		if(strpos($item->notes,'<p>') === false){ 
...
function gettestplan($projectid) {
	...
	$sql= "SELECT id,tc_external_id from tcversions where id in(SELECT id from nodes_hierarchy where  parent_id in (SELECT id from nodes_hierarchy where  parent_id in (SELECT id from nodes_hierarchy where  parent_id in (SELECT id from nodes_hierarchy where parent_id='$projectid'))));";//获取对应用例父节点
