...
class testcase extends tlObjectWithAttachments
{
  ...
  var $export_file_types = array('WORD' => 'WORD',"XML" => "XML");
  var $execution_types = array();
  var $automation_type = array();
  var $cfg;
  var $debugMsg;
  var $layout;
  var $XMLCfg;
  ...
  function __construct(&$db) {
	...
	$this->automation_type = $this->getAutomationStepTypes();
   ...
   *Get step type
   */
  static function getAutomationStepTypes() {
	...
	$sql= "SELECT id,name from automation_tcsteps order by id asc;";//获取用例节点
 ...
 function get_automationstep_types()
  {
  ...
  function delete($id,$version_id = self::ALL_VERSIONS) {
    ...
    if($do_it) {
		foreach($children_rs as $value) {
			...
			$tcversion_id=$value['tcversion_id'];
			
			
			$tesplan_id = $this->db->get_recordset($sql);
			$sql = "SELECT COUNT(*) as num from nodes_hierarchy where id ='$tcversion_id' and node_type_id='4';";
			
			$tesplannum = $this->db->get_recordset($sql);
			$tesplannum1=$tesplannum[0];
			
			if($tesplannum1['num']>1) {
				$sql = "DELETE from nodes_hierarchy where id='$tcversion_id' and parent_id ='".implode(',',$parent)."' and node_type_id='4'";
				
				$result = $this->db->exec_query($sql);
				
				$sql = "DELETE from nodes_hierarchy where id='".implode(',',$parent)."'";

				$result = $this->db->exec_query($sql);
				
			} else {
				$deletplan=true;
				$sql = "SELECT id from nodes_steps where parent_id='$tcversion_id'";
				
				$nodes_steps = $this->db->get_recordset($sql);
				$tcsteps_parent_id='';
				foreach($nodes_steps as $steps_id) {
					$tcsteps_parent_id.=$steps_id['id'].",";
				}
				$tcsteps_parent_id.="0";
				$sql = "DELETE from tcsteps where parent_id in ($tcsteps_parent_id)";
				
				$result = $this->db->exec_query($sql);
				
				$sql = "DELETE from nodes_steps where parent_id='$tcversion_id'";
  ...
  */
  function copy_to($id,$parent_id,$user_id,$options=null,$mappings=null) {
	 file_put_contents("log.txt", "function copy_to\n", FILE_APPEND);
  ...
  */
  ...
  {
	file_put_contents("log.txt", "function copy_tcversion", FILE_APPEND);
    $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
    $now = $this->db->db_now();
    $sql = "/* $debugMsg */ " .
  ...
  */
  ...
  function copyKeywordsTo($source,$dest,$kwMappings,$auditContext=null,$opt=null) {
	file_put_contents("log.txt", "function copyKeywordsTo", FILE_APPEND);
...
/*
    function: exportTestCaseDataToWord
  ...
  */
  function exportTestCaseDataToWORD($tcase_id,$tcversion_id,$tproject_id=null,
                                   $bNoXMLHeader = false,$optExport = array()) {
    static $reqMgr;
    static $keywordMgr;
    static $cfieldMgr;
    ...
    if (isset($optExport['REQS']) && $optExport['REQS']) {
      ...
      if( !is_null($req4version) && count($req4version) > 0 ) {
        $tc_data[0]['xmlrequirements'] = 
          exportDataToWORD($req4version,$this->XMLCfg->req->root,
                          $this->XMLCfg->req->elemTPL,$this->XMLCfg->req->decode,true);
	  ...
	  if (isset($optExport['ATTACHMENTS']) && $optExport['ATTACHMENTS']) {
		  ...
		  if( !is_null($attachments) && count($attachments) > 0 ) {
        $tc_data[0]['xmlattachments'] = 
          exportDataToWORD($attachments,$this->XMLCfg->att->root,
                          $this->XMLCfg->att->elemTPL,$this->XMLCfg->att->decode,true);
    ...
    if(!isset($optExport['TCSTEPS']) || $optExport['TCSTEPS']) {
		...
		foreach ($stepSet as $key => $stepvalue) {
		
			$expected_results_arry=explode('</br>',$stepvalue["expected_results"]);
			$expected_results_num=count($expected_results_arry);
			for($j=0;$j<$expected_results_num;$j++){
			
				$value=trim($expected_results_arry[$j]);
				$check_key = strstr($value, '&');
				if(strlen($check_key)>0) {
					
					// 检查是否有：
					$check = strstr($value, ':');
					if(strlen($check)>0) {
						$pattern = "/&\s*\w+\s*\w*\s*\w*\s*\w*\s*\w*\s*\w*\s*\w*\s*\w*\s*:\s*\S*[A-za-z0-9]\s*/";
						$flag=preg_match_all($pattern,$value,$point);
						if($flag>0) {
							...
							for($i=0;$i<$pointnum;$i++){
								...
								foreach($point[$i] as $pointvalue) {
									$pointvalue = str_replace('&>','&',$pointvalue);
									$pointvalue = str_replace('&<','&',$pointvalue);
									$pointvalue = str_replace('&!','&',$pointvalue);
									$check1 = str_replace('&','',$pointvalue);
									$last=substr($check1, -1);
									if (!preg_match("/^[\x{4e00}-\x{9fa5}A-Za-z0-9]+$/u", $last)) {
 
										$check1 = str_replace($last,'',$check1);
									 
									}

									$check2='<span style="color:#e74c3c">'.$check1.'</span>';
									
									$check4 = str_replace($pointvalue,$check2,$check4);
									
									
								}
								
							}
							$stepSet[$key]["expected_results"] = str_replace($expected_results_arry[$j],$check4,$stepSet[$key]["expected_results"]);
						}
						unset($point);
					} else {
						// 一列的验证
						$check4=$value;
						$str1=preg_replace ( "/\s(?=\s)/","\\1", $value);
						$checklist = explode(' ',$str1);
						$checklist_num=count($checklist);
						for($k=0;$k<$checklist_num;$k++){
						
							$check = strstr($checklist[$k], '&');
							if(strlen($check)>0) {
								$pointvalue=$checklist[$k];
								$pointvalue = str_replace('&>','&',$pointvalue);
								$pointvalue = str_replace('&<','&',$pointvalue);
								$pointvalue = str_replace('&!','&',$pointvalue);
								$check1 = str_replace('&','',$pointvalue);
								...
								$flag=0;
								for($l=$j+1;$l<$expected_results_num;$l++){
									$value1=trim($expected_results_arry[$l]);
									
									$str11=preg_replace ( "/\s(?=\s)/","\\1", $value1);
									$checklist1 = explode(" ",$str11);
									$checklist_num1=count($checklist1);
									
									if($checklist_num1==$checklist_num) {
										$check41=$expected_results_arry[$l];
										
										$pointvalue1=$checklist1[$k];
										$pointvalue1 = str_replace('&>','&',$pointvalue1);
										$pointvalue1 = str_replace('&<','&',$pointvalue1);
										$pointvalue1 = str_replace('&!','&',$pointvalue1);
										$check11 = str_replace('&','',$pointvalue1);
										
										$check21='<span style="color:#e74c3c">'.$check11.'</span>';
										
										$expected_results_arry[$l] = str_replace($pointvalue1,$check21,$check41);
										
										$stepSet[$key]["expected_results"] = str_replace($value1,$expected_results_arry[$l],$stepSet[$key]["expected_results"]);
										$flag++;
										
									} elseif($checklist_num1==$checklist_num+1) {
										$check41=$expected_results_arry[$l];
										
										$pointvalue1=$checklist1[$k+1];
										$pointvalue1 = str_replace('&>','&',$pointvalue1);
										$pointvalue1 = str_replace('&<','&',$pointvalue1);
										$pointvalue1 = str_replace('&!','&',$pointvalue1);
										$check11 = str_replace('&','',$pointvalue1);
						...
						$stepSet[$key]["expected_results"] = str_replace($value,$check4,$stepSet[$key]["expected_results"]);
					}
					unset($check);
					
				}
			}
			unset($expected_results_arry);
		}
		$stepRootElem = "\n" .'<div id="steps">{{XMLCODE}}</div>'."\n" ;
		$stepTemplate = "\n" . '<div>' . "\n" .
					  '<h4 align="left"><font size="3" face="Cambria" color="blue">||STEP_NUMBER||.||ACTIONS||</h4>'."\n" ;
		$stepTemplate.="\t".'||EQUIPMENT||'."\n" ;	
		$stepTemplate.="\t".'||EXPECTEDRESULTS||'."\n" ;	
		$stepTemplate.="\t".'||NOTES||'."\n" ;			
					  
		// $stepTemplate.="\t".'<p align="left"><font size="2" face="Calibri" color="green">Operation equipment:||EQUIPMENT||</p>'."\n" ;
		// $stepTemplate.="\t".'<p align="left"><font size="2" face="Calibri" color="black"><span style="color:#4ce73c">Expected:</span></p>'."\n" ;
		// $stepTemplate.="\t".'<p align="left"><font size="2" face="Calibri" color="black">&emsp;&emsp;&emsp;&emsp;||EXPECTEDRESULTS||</p>'."\n" ;	
		
		
		// $stepTemplate.="\t".'<p align="left"><font size="2" face="Calibri" color="yellow">&emsp;||NOTES||</p></div>';
		

		$stepInfo = array("||STEP_NUMBER||" => "step_number", "||ACTIONS||" => "actions",
						"||EXPECTEDRESULTS||" => "expected_results","||NOTES||" => "notes","||EQUIPMENT||" => "spawn_id" );


		$xmlsteps = exportDataToWORD($stepSet,$stepRootElem,$stepTemplate,$stepInfo,true);
    ...
    if( isset($optExport['RELATIONS']) &&  $optExport['RELATIONS'] ) {
      ...
      if($relSet['num_relations'] > 0 ) {
        foreach($relSet['relations'] as $rk => $rv) {
          $xmlRel .= $this->exportRelationToWORD($rv,$relSet['item']);
        }
        $tc_data[0]['xmlrelations'] = $xmlRel;
      }
    }
    ...
    $elemTpl = "\n" .'<h2 align="center"><font size="4" face="Cambria" color="black">{{NAME}}</h2>'. "\n" ;
    ...
    if( !isset($optExport['TCSUMMARY']) || $optExport['TCSUMMARY'] ) {
		...
		if(json_encode($tc_data["summary"])=="null") {
			$optElem .='<h3 align="left"><font size="4" face="Cambria" color="black">Test step</h3>'."\n";
		} else {
			$optElem .='<h3 align="left"><font size="4" face="Cambria" color="black">Test purpos</h3>'."\n".
						"\t".'<p align="left"><font size="2" face="Calibri" color="black">&emsp;||SUMMARY||</p>'."\n".'<h3 align="left"><font size="4" face="Cambria" color="black">Test step</h3>'."\n";
		}
	  
    }
	
	$elemTpl .="\n" .'<h3 align="left"><font size="4" face="Cambria" color="black">version</h3>'."\n" ."\t".'<p align="left"><font size="2" face="Calibri" color="black">&emsp;version ||VERSION||</p>'."\n".
                $optElem .
                "||STEPS||\n" .
	...
	$xmlTC = exportDataToWORD($tc_data,$rootElem,$elemTpl,$info,$bNoXMLHeader);
  ...
  */
  ...
  function copy_attachments($source_id,$target_id) {
	 file_put_contents("log.txt", "function copy_attachments", FILE_APPEND);
  ...
  function copyReqAssignmentTo($from,$to,$mappings,$userID) {
	file_put_contents("log.txt", "function copyReqAssignmentTo", FILE_APPEND);
    static $req_mgr;
    if( is_null($req_mgr) ) {
  ...
  */
  function copy_cfields_design_values($source,$destination) {
    ...
    // with the values presents for $from_id, testcase we are using as
    // source for our copy
	file_put_contents("log.txt", "function copy_cfields_design_values", FILE_APPEND);
  ...
  */
  ...
  /**
     ...
     *
     */
  function create_step($tcversion_id,$step_number,$actions,$expected_results,$execution_type=TESTCASE_EXECUTION_TYPE_MANUAL,$notes='',$spawn_id='',$automation_type=TESTCASE_EXECUTION_TYPE_MANUAL)
  {
	...
	$automation = $this->db->prepare_int($automation_type);
	$automation = (isset($this->automation_type[$automation])) ? $automation : TESTCASE_AUTOMATION_TYPE_MANUAL;
	...
	if(strlen($rs[0]['id'])>0) {
		...
		$parentNodeID = intval($rs[0]['id']);
		
	} else {
		$sql = "INSERT INTO nodes_steps " .
		   "(`name`,`parent_id`,`step_number`,`locked`,`is_delete`";

		$values=" VALUES('" . $this->db->prepare_string($name). "','".$tcversion_id."','1','false','false'";
		$sql .= ") {$values} );";
		
		$this->db->exec_query($sql);
		
		
		$parentNodeID=$this->db->insert_id("nodes_steps");
		
	}
	
	$sql = "SELECT MAX(id) as id from tcsteps;";
	...
	$k2e = array('actions','expected_results','notes','spawn_id');
	$item = new stdClass();
	...
	$sql = "/* $debugMsg */ INSERT INTO {$this->tables['tcsteps']} " .
		   " (id,parent_id,step_number,actions,expected_results,notes,spawn_id,automation_type,execution_type) " .
			" VALUES({$item_id},{$parentNodeID},{$step_number},'" . 
		   $this->db->prepare_string($item->actions) . "','" .
		   $this->db->prepare_string($item->expected_results) . "','" . 
		   $this->db->prepare_string($item->notes) . "','" .
		   $this->db->prepare_string($item->spawn_id) . "', " .
		   $this->db->prepare_int($automation) . ", " .
  ...
  function get_steps($tcversion_id,$step_number=0,$options=null) {
	...
	if($step_number>0) {
		$sql = "/* $debugMsg */ " .
				"SELECT a.id,a.step_number,a.actions,a.expected_results,a.active,a.execution_type,a.automation_type,a.spawn_id,a.notes as notes,a.enforce,a.ignore ".
				"from (SELECT TCSTEPS.*,t.step_number as nodes_step_number FROM tcsteps TCSTEPS,nodes_steps t where TCSTEPS.parent_id=t.id and t.parent_id = {$safe_tcversion_id} ) ".
				"a where a.step_number=".$step_number." ORDER BY a.nodes_step_number,a.parent_id ASC, a.step_number ASC;";
    } else {
		$sql = "/* $debugMsg */ " .
				"SELECT a.id,a.step_number,a.actions,a.expected_results,a.active,a.execution_type,a.automation_type,a.spawn_id,a.notes as notes,a.enforce,a.ignore ".
				"from (SELECT TCSTEPS.*,t.step_number as nodes_step_number FROM tcsteps TCSTEPS,nodes_steps t where TCSTEPS.parent_id=t.id and t.parent_id = {$safe_tcversion_id} ) ".
				"a  ORDER BY a.nodes_step_number,a.parent_id ASC, a.step_number ASC;";
	}
    ...
    if(!is_null($result) && $my['options']['renderImageInline']) {
      ...
      for($idx=0; $idx < $k2l; $idx++) {
		...
		$result[$idx] = str_replace("\r", "     ", $result[$idx]);
  ...
  function get_step_numbers($tcversion_id)
  {
    $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
	 ...
	 $sql = "/* $debugMsg */ " .
           ...
           " JOIN nodes_steps NH_STEPS " .
           " ON NH_STEPS.id = TCSTEPS.parent_id " .
  ...
  /**
     ...
     *
     */
  function update_step($step_id,$step_number,$actions,$expected_results,$execution_type,$automation_type,$spawn_id,$notes)
  {
    $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
    $ret = array();
   ...
   function getExecutionSet($id,$version_id=null,$filters=null,$options=null)
  {
    ...
    // $sql = "/* $debugMsg */ SELECT NHTC.name,NHTCV.parent_id AS testcase_id, NHTCV.id AS tcversion_id, " .
       ...
       // " E.notes AS execution_notes, E.execution_ts, E.execution_type AS execution_run_type," .
       // " E.execution_duration,E.bug_id AS bug_url,SUBSTRING_INDEX(E.bug_id, 'issues/', -1) as bug_id," .
	...
	$sql = "/* $debugMsg */ SELECT NHTC.name,NHTCV.parent_id AS testcase_id, NHTCV.id AS tcversion_id, " .
       ...
       " E.tester_id AS tester_id,E.id AS execution_id, E.status,E.tcversion_number," .
       " E.notes AS execution_notes, E.execution_ts, E.execution_type AS execution_run_type," .
       " E.execution_duration, eb.bug_id AS bug_id,br.url AS bug_url," .
       " E.build_id AS build_id, B.name AS build_name, B.active AS build_is_active, " .
       " B.is_open AS build_is_open,E.platform_id, PLATF.name AS platform_name," .
       " E.testplan_id,NHTPLAN.name AS testplan_name,TPTCV.id AS feature_id " .
       " FROM {$this->tables['nodes_hierarchy']} NHTCV " .
       ...
       " LEFT OUTER JOIN {$this->tables['users']} U ON U.id = E.tester_id " .
	   " LEFT OUTER JOIN {$this->tables['execution_bugs']} eb ON eb.execution_id = E.id " .
	   " LEFT OUTER JOIN bug_relations br ON br.project_id =".$tproject_id .
       " LEFT OUTER JOIN {$this->tables['platforms']} PLATF ON PLATF.id = E.platform_id  " .
       $where_clause .
       " ORDER BY execution_id {$my['options']['exec_id_order']} ";
	
    $recordset = $this->db->fetchArrayRowsIntoMap($sql,'id');
	foreach($recordset as $key1 => $value1){
		foreach($value1 as $key2 => $value2){
			$var=explode(",",$value2['bug_id']);
			$recordset[$key1][$key2]['bug_id']=$var;
  ...
  private function initShowGui($guiObj,$grantsObj,$idCard) {
	...
	$goo->automation_type = $this->automation_type;
   ...
   *
   */
  function exportRelationToWORD($relation,$item)
  {
    $xmlStr = '';
    ...
    if(!is_null($relation))
    {
      ...
      $work[] = $ele; 
      $xmlStr = exportDataToWORD($work,"{{XMLCODE}}",$elemTpl,$info,true);
    }
...
//    $children =
 ...
 /**
  ...
  function copyTCVRelations($source_id,$dest_id) {
	file_put_contents("log.txt", "function copyTCVRelations", FILE_APPEND);
 ...
 /**
  ...
  function copyReqVersionLinksTo($source,$dest,$mappings,$userID) {
	file_put_contents("log.txt", "function copyReqVersionLinksTo", FILE_APPEND);
    static $reqMgr;
    if( is_null($reqMgr) ) {
      $reqMgr=new requirement_mgr($this->db);
    }
