...
if(isset($_REQUEST['notes'])) {
	...
	if(strlen(trim($notes))==0) {
		...
		echo "<script>alert('提示：请给执行步骤添加相应的注释说明！\\nwarning：Please add notes for the steps!');location.href='".$_SERVER["HTTP_REFERER"]."';</script>"; 
		// exit();
	} 
...
switch($args->doAction) {
  ...
  case "doResequenceSteps":
    $testCaseEditorKeys = array('steps' => 'steps', 'expected_results' => 'expected_results', 'notes' => 'notes');
  break;
...
function init_args(&$cfgObj,$otName,&$tcaseMgr) {
  ...
  $args->spawn_id = isset($_REQUEST['spawn_id']) ? $_REQUEST['spawn_id'] : null;
  ...
  $args->automation_type = isset($_REQUEST['automation_type']) ? $_REQUEST['automation_type'] : TESTCASE_EXECUTION_TYPE_MANUAL;
  ...
  // Multiple Test Case Steps Feature stepnumber
...
function createWebEditors($basehref,$editorCfg,$editorSet=null)
{
    ...
    $cols = array('steps' => array('horizontal' => 28, 'vertical' => 44),
				  'notes' => array('horizontal' => 38, 'vertical' => 44),
    ...
    $owe->cfg = array('summary' => array('rows'=> null,'cols' => null),
					  ...
					  'notes' => array('rows'=> null,'cols' => $cols['notes'][$layout]) ,
...
function getCfg()
{
    ...
    $cfg->editorKeys->step = array('steps' => true, 'expected_results' => true,'notes' => true);    
