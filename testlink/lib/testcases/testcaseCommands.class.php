...
class testcaseCommands {
  ...
  function __construct(&$db,&$userObj,$tproject_id) {
	...
	$this->automation_type = $this->tcaseMgr->get_automationstep_types();
  ...
  function initGuiBean(&$argsObj) {
	...
	$obj->automation_type = $this->automation_type;
  ...
  */
  ...
  function createStep(&$argsObj,$request) {
	...
	$guiObj->step_automation_type = $guiObj->testcase['automation_type']; // BUGID 3810
  ...
  function doCreateStep(&$argsObj,$request,$doAndExit=false) {
	...
	$guiObj->step_automation_type = $argsObj->automation_type;
    $guiObj->tcversion_id = $argsObj->tcversion_id;
    $this->initTestCaseBasicInfo($argsObj,$guiObj);
    $guiObj->main_descr = sprintf(lang_get('create_step'), $guiObj->testcase['tc_external_id'] . ':' . 
                                  $guiObj->testcase['name'], $guiObj->testcase['version']); 
    ...
    $op = $this->tcaseMgr->create_step($argsObj->tcversion_id,$new_step,
                                       $argsObj->steps,$argsObj->expected_results,$argsObj->exec_type,$argsObj->notes,$argsObj->spawn_id,$argsObj->automation_type);  
    ...
    if( $op['status_ok'] )
    {
	  ...
	  $guiObj->step_automation_type = $guiObj->testcase['automation_type'];
      $guiObj->cleanUpWebEditor = true;
  ...
  function editStep(&$argsObj,$request)
  {
    ...
    $oWebEditorKeys = array('steps' => 'actions', 'expected_results' => 'expected_results', 'notes' => 'notes');
	...
	$guiObj->step_automation_type = $stepInfo['automation_type'];
  ...
  function doUpdateStep(&$argsObj,$request) {
	...
	$op = $this->tcaseMgr->update_step($argsObj->step_id,$argsObj->step_number,$argsObj->steps,
									   $argsObj->expected_results,$argsObj->exec_type,$argsObj->automation_type,$argsObj->spawn_id,$argsObj->notes);    
  ...
  function doCopyStep(&$argsObj,$request) {
    ...
    $op = $this->tcaseMgr->create_step($argsObj->tcversion_id,$new_step,$source_info['actions'],
                                       $source_info['expected_results'],$source_info['notes'],$source_info['spawn_id'],$source_info['automation_type'],$source_info['execution_type']);      
    ...
    if( $op['status_ok'] )
    {
	  ...
	  $guiObj->step_automation_type = TESTCASE_AUTOMATION_TYPE_MANUAL;
      $this->tcaseMgr->update_last_modified($argsObj->tcversion_id,$argsObj->user_id);
      $this->initTestCaseBasicInfo($argsObj,$guiObj);
    }  
