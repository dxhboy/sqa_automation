...
if ($args->doExport)
{
  ...
  switch($args->exportType)
  {
	...
	case 'WORD':
		...
		if ($gui->oneTestCaseExport)
		{
			$gui->export_filename = $tcinfo['name']."_". date('Ymd', time()) ."_". '.doc';
			$pfn = 'exportTestCaseDataToWORD';
		} else {
			$gui->export_filename = $node_info['name']."_". date('Ymd', time()) ."_". '.doc';
			$pfn = 'exportTestSuiteDataToWord';
		}
		...
		if ($pfn)
		{
			...
			{
			  $args->optExport['RELATIONS'] = true;
			  $args->optExport['ROOTELEM'] = '<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word" xmlns="http://www.w3.org/TR/REC-html40"><head>/n<STYLE>/nBR.page { page-break-after: always }/n</STYLE>/n<meta charset="UTF-8">/n</head>/n<body>/n{{XMLCODE}}/n</body>/n</html>';
			  
			  $content = $tcase_mgr->$pfn($args->tcase_id,$args->tcversion_id,$args->tproject_id,null,$args->optExport);
			}
			else
			{
			  $content = '<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word" xmlns="http://www.w3.org/TR/REC-html40"><head>/n<STYLE>/nBR.page { page-break-after: always }/n</STYLE>/n<meta charset="UTF-8">/n</head>/n<body>';
			  $content .= $tsuite_mgr->$pfn($args->container_id,$args->tproject_id,$args->optExport);
			  $content .='/n</body>/n</html>';
			}
			$result = str_replace('<?xml version="1.0" encoding="UTF-8"?>',"",$content);
			$result = str_replace('</br>','</p><p align="left"><font size="2" face="Calibri" color="black">&emsp;&emsp;&emsp;&emsp;',$result);
			$result = str_replace('<p align="left"><font size="2" face="Calibri" color="green">Operation equipment:None</p>','',$result);
			// file_put_contents("log.txt", "\n result:\n".$result, FILE_APPEND);
			
			downloadContentsToWORD($result,$gui->export_filename);
			exit();
		}
    break;
  }
