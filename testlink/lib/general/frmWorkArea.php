...
$aa_tfp = array( 
	 ...
	 'setTc' => array('lib/testcases/listTestCases.php?feature=edit_tc',
                       'lib/testcases/archiveData.php?edit=setTc&id='),
	 ...
	 'automationtest' => 'automation/mainPage.tpl'
);
...
// $testprojectID = isset($_SESSION['testprojectID']) ? intval($_SESSION['testprojectID']) : 0;
 ...
 if($showFeature == 'automationTest')
{
	$tplEngine = new Smarty();
	$path= dirname(dirname(dirname(__FILE__))).DIRECTORY_SEPARATOR.'automation\mainPage.tpl';

	// $dirpath=dirname(dirname(dirname(__FILE__)));
	$args->toppath='http://'.$_SERVER['HTTP_HOST'].'/testlink/automation/top.html';
	$args->leftpath='http://'.$_SERVER['HTTP_HOST'].'/testlink/automation/leftmenu.html';
	$args->mainpath='http://'.$_SERVER['HTTP_HOST'].'/testlink/automation/main.html';
	// file_put_contents("log.txt",  $dirpath."\n", FILE_APPEND);
	
	// $args->projectid = isset($_SESSION['testprojectID']) ? intval($_SESSION['testprojectID']) : 0;
	// file_put_contents("log.txt",  $args->projectid."\n", FILE_APPEND);
	$tplEngine->assign('args', $args);
	
	$tplEngine->display($path);
