...
$_SESSION['expiretime'] = time() + 43200;
/**
 * 
 *
 */
  ...
  if($op['status'] == tl::OK)
  {
    ...
    else 
    {
      ...
      else
      {
		...
		if ($_REQUEST['project_manage_platform']) {
			$rul = $_SESSION['basehref'] .
			         "index.php?caller=login&viewer={$argsObj->viewer}&type=project_manage_platform" . $ad;
		} else {
			$rul = $_SESSION['basehref'] .
			         "index.php?caller=login&viewer={$argsObj->viewer}&type=testlink" . $ad;
		}
		redirect($rul);
      }
