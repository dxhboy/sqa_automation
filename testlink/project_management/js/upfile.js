	...
	xhr.onreadystatechange = function () {//请求后的回调接口，可将请求成功后要执行的程序写在其中
		if (xhr.readyState == 4 && xhr.status == 200) {//验证请求是否发送成功
			...
			if(type=="image") {
				...
				$("#picpath").val(htmldata);
			} else {
				var htmldata=$("#filepath").val();
				...
				$("#filepath").val(htmldata);
			}
			var nametext=document.getElementById("filenametext").innerText;	
			...
			document.getElementById("filenametext").innerText=nametext;
		}
	};
	
}

function paste_imgs(e) {
