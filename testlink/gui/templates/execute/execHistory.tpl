...
{lang_get var='labels' 
          s='title_test_case,th_test_case_id,version,date_time_run,platform,test_exec_by,
             exec_status,testcaseversion,attachment_mgmt,deleted_user,build,testplan,testcasebugid,
             execution_type_manual,execution_type_auto,run_mode,exec_notes,edit_bug_id,
...
function openEditbugidWindow(exec_id,tcversion_id,tplan_id,tproject_id)
{
  var target_url = "lib/execute/editExecutionbugid.php";
  ...
  {if $gui->warning_msg == ''}
    ...
    <table cellspacing="0" class="exec_history">
      <tr>
		...
		<th style="text-align:center">{$labels.testcasebugid}</th>
          ...
          <tr style="border-top:1px solid black; background-color: {$bg_color}">
            ...
            *}
			...
			<td  align="center">
				...
				{if $gui->grants->exec_edit_notes[$tcv_exec.testplan_id]}
					<img src="{$smarty.const.TL_THEME_IMG_DIR}/note_edit.png" style="vertical-align:middle" 
						 title="{$labels.edit_bug_id}" onclick="javascript: openEditbugidWindow(
						 {$tcv_exec.execution_id},{$tcv_exec.id},{$tcv_exec.testplan_id},{$gui->tproject_id});">
				{/if}
				 
				{foreach item=bugid from=$tcv_exec.bug_id}
					<a href="{$tcv_exec.bug_url}{$bugid}" target="_blank">{$bugid}</a>
				{/foreach}
